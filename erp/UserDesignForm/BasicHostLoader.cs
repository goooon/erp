namespace UserDesignForm
{
	...
	public class BasicHostLoader: BasicDesignerLoader
	{
        ...
        public BasicHostLoader(Type rootComponentType, string formName)
		{
        ...
        private void ReadEvent(XmlNode childNode, object instance, ArrayList errors)
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show(ex.Message, "´íÎó26");
            }
        }
            ...
            else
            {
                ...
                if (ReadValue(node, prop.Converter, errors, out value))
                {
                    ...
                    if (value != null)
                    {
                        ...
                        {
                            MessageBox.Show(ex.Message, "´íÎóbb");
                            errors.Add(ex.Message);
                        }
                    }
                }
        ...
        private bool ReadValue(XmlNode node, TypeConverter converter, ArrayList errors, out object value)
        {
            try
            {
                foreach (XmlNode child in node.ChildNodes)
                {
                    ...
                    {
                        MessageBox.Show(string.Format("Unexpected element type {0}", child.Name), "´íÎó");
            ...
            {
                errors.Add(ex.Message);
                MessageBox.Show(ex.Message, "´íÎóa");
                value = null;
                return false;
            }
        }
